...
var self = module.exports = { // eslint-disable-line
  cloud: {
	...
	getIcons: async function () {
    	const data = await http.json(`${apiEndpoint}/icons?page=0&page_size=10000&fields=id,title,code,thumb`);
    	let result = data.data;
		for(let page = 1; page < data.meta.page_count; page++) {
        	let pagedata = await http.json(`${apiEndpoint}/icons?page=${page}&page_size=10000&fields=id,title,code,thumb`);
        	result = result.concat(pagedata.data);
		}
     	console.log("Fuck me, I'm finally finished caching icons!")
     	// console.log(result)
     	return result;
    },
  ...
  device: {
    doAppAction: function (ip, apiKey, appId, action, params) {
      return self.device.getWidgets(ip, apiKey).then(widgets => {
        ...
        var options = {
          uri: `http://dev:${apiKey}@${ip}:8080/api/v2/device/apps/${appId}/widgets/${widgetId}/action`,
          json: { id: action }
        }
    ...
    sendNotification: function (ip, apiKey, priority, icontype, text, icon, sound, repeat) {
      var options = {
      ...
      if (sound) { options.json.model.sound = { category: 'notifications', id: sound, repeat: repeat } }
