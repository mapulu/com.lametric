...
function cacheIcons () {
  lametric.cloud.getIcons().then(icons => {
    ...
    util.errorLog('Something went wrong getting icons:', reason)
  })
}
...
function onAutoCompleteWidget (callback, args) {
  ...
  util.debugLog('building widgetCache...', args)
  Homey.manager('drivers').getDriver('lametric').getSettings(args.args.device.data, (error, settings) => {
    if (error) return callback(error)
    lametric.device.getWidgets(settings.ipv4, args.args.device.data.apiKey).then(widgets => {
      widgetsCache[args.args.device.mac] = {
        list: widgets.map(widget => ({id: widget.widget, name: widget.title, app: widget.package})),
        buildTime: new Date()
      }
      return callback(null, filterAutocomplete(widgetsCache[args.args.device.data.mac].list, args.query))
    }).catch(error => {
      callback(error)
    })
  })
...
function onNotificationText (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.device.sendNotification(settings.ipv4, args.device.apiKey, args.priority, args.icontype, args.text, null, null).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextIcon (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.device.sendNotification(settings.ipv4, args.device.apiKey, args.priority, args.icontype, args.text, args.icon.code, null).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextIconSound (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.device.sendNotification(settings.ipv4, args.device.apiKey, args.priority, args.icontype, args.text, args.icon.code, args.sound.id, args.repeatcount).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
...
function onNotificationTextSound (callback, args) {
  Homey.manager('drivers').getDriver('lametric').getSettings(args.device, (error, settings) => {
    if (error) return callback(error)
    lametric.device.sendNotification(settings.ipv4, args.device.apiKey, args.priority, args.icontype, args.text, null, args.sound.id, args.repeatcount).then(result => {
      callback(null, true)
    }).catch(callback)
  })
}
